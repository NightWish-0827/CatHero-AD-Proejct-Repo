...
public class MasterInstaller : MonoBehaviour
{
    ...
    public void RefreshRegistry()
    {
        ...
        foreach (var comp in allComponents)
        {
            ...
            {
                // 같은 타입의 [Referral] 이 여러 개 있으면 경고 (첫 번째만 사용)
                if (!seenTypes.Add(type))
                {
                    Debug.LogWarning($"[MasterInstaller] Multiple [Referral] components of type {type.Name} found in scene. Only the first one will be used in global registry.");
                }
